...
function createBall(){
    ...
    if (Math.round(Math.random()) == 1) {
        ballXDirection = 1;     // ball moves right
    }
    else{
        ballXDirection = -1;    // ball moves left
    }
    if (Math.round(Math.random()) == 1) {
        ballYDirection = 1;     // ball moves 
    }
    else{
        ballYDirection = -1;    // ball moves 
    }
...
function checkCollision(){
    ...
    if (ballX <= (paddle1.x + paddle1.width + ballRadius)) {
        if (ballY > paddle1.y && ballY < paddle1.y + paddle1.height) {
            ballX = (paddle1.x + paddle1.width) + ballRadius;   // if the ball gets struck in corner
            ballXDirection *= -1;
            ballSpeed += 1;
        }
    }
